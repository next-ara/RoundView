...
public class DampingReboundNestedScrollView extends NestedScrollView {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                //记录childView的初始位置
                this.topRect.set(this.childView.getLeft(), this.childView.getTop(),
                        this.childView.getRight(), this.childView.getBottom());
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if ((!this.childView.canScrollVertically(-1) && (this.currentY - this.startY) > 0) || (!this.childView.canScrollVertically(1) && (this.currentY - this.startY) < 0)) {
                    ...
                    this.childView.layout(this.topRect.left, (int) (this.topRect.top + this.moveHeight), this.topRect.right,
                            (int) (this.topRect.bottom + this.moveHeight));
                }
                break;
            case MotionEvent.ACTION_UP:
                if (!this.topRect.isEmpty()) {
                    ...
                    //子控件回到初始位置
                    this.childView.layout(this.topRect.left, this.topRect.top, this.topRect.right,
                            this.topRect.bottom);
                }
    ...
    private void upDownMoveAnimation() {
        TranslateAnimation animation = new TranslateAnimation(0.0f, 0.0f,
                this.childView.getTop(), this.topRect.top);
